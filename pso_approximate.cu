...
__device__ double random_v(int *next);
...
__global__ void move(double *position_d, double *velocity_d, double *fitness_d, 
    ...
    if(idx < particle_cnt_d){
        for(unsigned int i = 0; i < max_iter_d; i++){
            ...
            if(pfitness > gbest->fitness){
                gbest->position = ppos;
                gbest->fitness  = pfitness;
            }
        }
        position_d[idx] = pos;
        velocity_d[idx] = v;
        fitness_d[idx]  = fitness;
        pbest_fit_d[tidx] = pfitness;
        pbest_pos_d[tidx] = ppos;
    }
}
...
int main(int argc, char **argv){
    ...
    float exe_time; //float init_time;
    // 變數宣告
    ...
    move<<<args.blocks_per_grid, args.threads_per_block>>>
            (position_d, velocity_d, fitness_d, pbest_pos_d, pbest_fit_d, gbest_d, lock_d);
